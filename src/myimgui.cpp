...
int myimgui::CreateWindow_Violet(LPCWSTR class_name, LPCWSTR title_name, myimgui::myFun Fun, const char* Fontsname, float Fonts_size, bool Synclnterval, int Menustyle)
{
    ...
    mywindow.hwnd = ::CreateWindowExW(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW,
                                      wc.lpszClassName, L"Violet DirectX11 v1.1", WS_POPUP, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
...
void myimgui::UpdateWindow()
{
    ...
    //查找目标窗口
    gamewindow.hwnd = FindWindowW(gamewindow.ClassName, gamewindow.TitleName);

    //获取进程PID
    GetWindowThreadProcessId(gamewindow.hwnd, &gamewindow.pid);

    //打开进程
    gamewindow.handle = OpenProcess(PROCESS_ALL_ACCESS, true, gamewindow.pid);

    if (gamewindow.handle == NULL)
    {
        // 获取错误代码
